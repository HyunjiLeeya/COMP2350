...
-- CREATE DATABASE COMP2350_zA2W05TeamB; 
-- USE COMP2350_zA2WO5TeamB;
...
('GC1004', 10, 50.00, TRUE, '2024-11-25'), -- active but expired
('GC1005', 13,   85.00, FALSE, '2026-10-23'), -- inactive but in date
('GC1006', 20,   12.00, TRUE, '2025-10-09'); -- expires today.
...
-- 2.1 
DELIMITER // 
CREATE FUNCTION calcLoyaltyPoints(orderID INT) -- orderID as parameter because its looking for given orders.
RETURNS INT -- INT for pointsEarnt
	DETERMINISTIC
	BEGIN
    DECLARE pointsEarnt INT; -- declaring tracker pointsEarnt
    SELECT sum(pointsEarned) INTO pointsEarnt -- summing points earnt into the tracker variable.
    FROM LoyaltyTransaction 
    WHERE LoyaltyTransaction.orderID = orderID; 
    
    IF pointsEarnt IS NULL THEN -- checks if order has no points, if so , points are set to 0.
    SET pointsEarnt = 0;
    end IF;
    RETURN pointsEarnt; -- return the pointsEarnt from specific order
    END;
    //
    DELIMITER ;
    
    -- These are to check the function works as expected
    select calcLoyaltyPoints(1) AS 'Points Earned';
	select calcLoyaltyPoints(3) AS 'Points Earned ';
    select calcLoyaltyPoints(5) AS 'Points Earned';
    select calcLoyaltyPoints(6) AS 'Points Earned';
    select calcLoyaltyPoints(8) AS 'Points Earned';
    select calcLoyaltyPoints(9) AS 'Points Earned';

-- 2.2
DELIMITER //
CREATE FUNCTION isGiftCardValid(giftCardCode VARCHAR(20))
RETURNS varchar(20) -- returns as text around 20 characters.
DETERMINISTIC 
	BEGIN
		DECLARE validCheck VARCHAR(8); -- tracking variable
        DECLARE activeCheck BOOLEAN; -- isActive alias
        DECLARE expiry DATE; -- expirationDate alias
        SELECT isActive, expirationDate 
        INTO activeCheck, expiry -- uses the alias instead of isActive and expirationDate.
		FROM GiftCard 
        WHERE GiftCard.giftCardCode = giftCardCode; -- GiftCard.giftCardCode as was getting "result consisted of more than 1 row" error.
                                                    -- giftcard. allows MYSQL workbench to differentiate the two (param and in the table)
       
       IF activeCheck IS TRUE AND expiry > CURRENT_DATE THEN -- checks if giftcards are active and not expired (expiry is more than current date) as if its less, it is invalid.
        SET validCheck = 'VALID'; -- updates valid check
        ELSE SET validCheck = 'INVALID'; -- if it does not meet both criteria above, giftcard is invalid.
        END IF;
        RETURN validCheck;
...
DELIMITER ;
	...
	-- these are to check the function works as expected.
    select isGiftCardValid('GC1001') AS 'Valid?';
    select isGiftCardValid('GC1002') AS 'Valid?';
    select isGiftCardValid('GC1003') AS 'Valid?';
    select isGiftCardValid('GC1004') AS 'Valid?'; -- active and expired
    select isGiftCardValid('GC1005') AS 'Valid?'; -- inactive but in date
    select isGiftCardValid('GC1006') AS 'Valid?'; -- expires today
